...
class MCPClient:
    ...
    async def connect_to_server(self, server_path: str):
        ...
        Args:
            server_path: Either a URL (http://host:port) or a path to a script to run
        """
        try:
            # Try parsing as URL first
            url = urlparse(server_path)
            if url.scheme in ['http', 'https']:
                ...
                # Not a URL - treat as script path
    ...
    async def process_query(self, query: str) -> str:
        """Process user query using the Gemini API and execute tools if needed."""
...
def clean_schema(schema):
    """Clean up JSON schema by removing title fields."""
...
async def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  server_path can be:")
        print("  - A URL (e.g., http://localhost:9876)")
        print("  - A script path (e.g., ../server.py)")
        sys.exit(1)
